...
export const DemoDataProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const updateDeal = useCallback((id: string, patch: Partial<DealWithAssignees>) => setDeals(prev => prev.map(d => d.id === id ? { ...d, ...patch } : d)), []);
    ...
    const updateBillingOp = useCallback((id: string, patch: Partial<BillingDoc>) => setBillingOps(prev => prev.map(o => o.$id === id ? { ...o, ...patch } : o)), []);
    const deleteBillingOp = useCallback((id: string) => setBillingOps(prev => prev.filter(o => o.$id !== id)), []);

    const addMessage = useCallback((msg: Message) => {
        setMessages(prev => prev.some(existing => existing.$id === msg.$id) ? prev : [msg, ...prev]);
    }, []);
    const updateMessage = useCallback((id: string, patch: Partial<Message>) => setMessages(prev => prev.map(m => m.$id === id ? { ...m, ...patch } : m)), []);
    ...
    const updateNote = useCallback((id: string, patch: Partial<NoteData>) => setNotes(prev => prev.map(n => n.$id === id ? { ...n, ...patch } : n)), []);
