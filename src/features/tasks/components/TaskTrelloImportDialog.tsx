...
interface TaskTrelloImportDialogProps {
    open: boolean
    onOpenChange: (open: boolean) => void
    targetStatus: string
    targetStatusLabel: string
    onImport: (cards: TrelloCard[], status: string) => Promise<void>
}

type Step = 'auth' | 'waiting' | 'board' | 'list' | 'cards' | 'importing'

const TaskTrelloImportDialog = ({ open, onOpenChange, targetStatus, targetStatusLabel, onImport }: TaskTrelloImportDialogProps) => {
    ...
    const handleImport = async () => {
        ...
        setStep('importing')
        try {
            await onImport(toImport, targetStatus)
        ...
        if (step === 'auth') {
            return (
                <div className="space-y-4 py-2">
                    ...
                    <Button className="w-full gap-2" onClick={handleConnect} disabled={!authUrlData?.data} variant="outline">
                        {!authUrlData?.data
        ...
        if (step === 'cards' || step === 'importing') {
            ...
            return (
                <div className="space-y-2">
                    ...
                    <div className="max-h-64 space-y-1.5 overflow-y-auto overflow-x-hidden pr-1">
                        {(cards ?? []).map((card) => (
                            <label
                                key={card.id}
                                className={cn(
                                    "flex min-w-0 w-full cursor-pointer items-start gap-3 overflow-hidden rounded-md border px-3 py-2.5 transition-colors hover:bg-accent",
    ...
    return (
        <DialogContainer
            ...
            description={
                breadcrumb.length > 0
                    ? breadcrumb.join(' › ')
                    : t('trello.dialog-subtitle', { column: targetStatusLabel })
            }
        >
            <div className="space-y-4 min-w-0">
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <Image src="/integrations/trello.png" alt="Trello" width={14} height={14} className="shrink-0" />
...
export default TaskTrelloImportDialog
